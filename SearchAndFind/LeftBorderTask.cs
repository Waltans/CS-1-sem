...
namespace Autocomplete
{
    ...
    public class LeftBorderTask
    {
        ...
        public static int GetLeftBorderIndex(IReadOnlyList<string> phrases, string prefix, int left, int right)
        {
            ...
            var middleElement  = left + (right - left) / 2;
            if (string.Compare(prefix, phrases[middleElement], StringComparison.OrdinalIgnoreCase) < 0
                || phrases[middleElement].StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
                return GetLeftBorderIndex(phrases, prefix, left, middleElement);
            return GetLeftBorderIndex(phrases, prefix, middleElement, right);
        }
    }
}
