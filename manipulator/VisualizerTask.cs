...
namespace Manipulation
{
    public static class VisualizerTask
    {
        ...
        {
            PointF totalCoord = ConvertWindowToMath(new PointF(e.X, e.Y), GetShoulderPos(form));
            X = totalCoord.X;
            Y = totalCoord.Y;
            UpdateManipulator();
            form.Invalidate();
        ...
        public static void UpdateManipulator()
        {
            ...
            if (double.IsNaN(angles[0]) || double.IsNaN(angles[1]) || double.IsNaN(angles[2])) return;
        ...
        public static void DrawManipulator(Graphics graphics, PointF shoulderPos)
        {
            ...
            for (int i = 0; i <= joints.Length; i++)
                graphics.FillEllipse(JointBrush, points[i].X , points[i].Y,2, 2);
        }
            ...
            PointF shoulderPos, PointF[] joints)
        {
            ...
            graphics.FillEllipse(reachableBrush,
                windowCenter.X - rmax, windowCenter.Y - rmax,2 * rmax, 2 * rmax);
            graphics.FillEllipse(unreachableBrush,
                windowCenter.X - rmin, windowCenter.Y - rmin,2 * rmin, 2 * rmin);
        }
